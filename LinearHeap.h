...
public:
	...
	bool get_max(ui &id, ui &key) {// return true if success, return false otherwise
		while(min_key <= max_key&&head_s[max_key] == n) -- max_key;
		if(min_key > max_key) return false;
		id = head_s[max_key];
		key = max_key;
		assert(key_s[id] == key);
	...
	bool pop_max(ui &id, ui &key) {// return true if success, return false otherwise
		while(min_key <= max_key&&head_s[max_key] == n) -- max_key;
		if(min_key > max_key) return false;
		id = head_s[max_key];
		key = max_key;
		assert(key_s[id] == key);
		key_s[id] = -1;
		head_s[max_key] = next_s[id];
		if(head_s[max_key] != n) pre_s[head_s[max_key]] = n;
		return true;
	}
	...
	ui decrement(ui id, ui dec) {
		//assert(key_s[id] >= dec);
		if(key_s[id] > key_cap || key_s[id] < 0) return 0;
